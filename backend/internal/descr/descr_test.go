...
func TestNormalizeAndStrip(t *testing.T) {
	src := `  <img src="https://example.org/pic/logo.png" alt="logo" /><br />
<fieldset>
...
func TestDescrBuilderFileListText(t *testing.T) {
	...
	out := b.Build(map[string]any{
		...
		"descr": `<fieldset><b>片名:</b>Have a Feast</fieldset><br />来自 example.org 发布`,
	}, map[string]any{
	...
	if strings.Contains(out, "来自 example.org") {
...
// TestImdbValidationAndEscape 加固:imdb 值必须先过 `tt\d{6,}` 白名单,否则不嵌入。
func TestImdbValidationAndEscape(t *testing.T) {
	b := NewDescrBuilder()
	// 合法 imdb 正常嵌入。
	out := b.Build(map[string]any{
		"title": "Test",
	...
	if !strings.Contains(out, "tt6485574") {
		t.Errorf("valid imdb not embedded: %s", out)
	}
	// 非法 imdb(注入/带特殊字符)不得嵌入,也不得把注入带进输出。
	out = b.Build(map[string]any{
		"title": "Test",
		"descr": "<b>剧情</b>",
		"imdb":  `tt6485574"><script>alert(1)</script>`,
	}, nil)
	if strings.Contains(out, "<script>") || strings.Contains(out, "alert(1)") {
		t.Errorf("invalid imdb leaked into output: %s", out)
	}
	// 太短(不足 6 位数字)也不得嵌入。
	out = b.Build(map[string]any{
		"title": "Test",
	...
	if strings.Contains(out, "tt123") {
		t.Errorf("short imdb leaked into output: %s", out)
	}
}

// TestEventAttrSanitize 加固:对外部 HTML 去掉 on*= 事件属性与 javascript: 伪协议。
func TestEventAttrSanitize(t *testing.T) {
	src := `<div><a href="javascript:alert(1)" onclick="evil()">点击</a><img src="x" onerror="evil()" onload="bad()"></div>`
	out := NormalizeDescription(src)
	if strings.Contains(out, "onclick") || strings.Contains(out, "onerror") || strings.Contains(out, "onload") {
		t.Errorf("event attrs not removed: %s", out)
	}
	if strings.Contains(out, "javascript:") {
		t.Errorf("javascript: URL not removed: %s", out)
	}
	if !strings.Contains(out, "点击") {
		t.Errorf("text content not preserved: %s", out)
	}
}
