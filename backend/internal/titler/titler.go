...
// 结尾制作组:用 `-` / `_` 分隔,不用 `.` —— `.` 粘连的是技术 token(如
// `AAC2.0.H.264`),旧版把 `.` 当分隔符会误吞视频/音频编码与声道 token。
// 注意分隔符不含空格 —— 空格分隔的是标题单词,不是制作组。
var groupTokenShapeRe = regexp.MustCompile(`^[A-Za-z][A-Za-z0-9.@]{0,24}$`)

// knownHyphenTokens 归一化后的「含连字符技术 token」:标题尾部的 `-` 若属于
// 这些 token 内部(如 `WEB-DL`/`DTS-HD`/`Blu-Ray`),不能当作制作组分隔符。
var knownHyphenTokens = map[string]bool{
	"WEBDL": true, "WEBRIP": true, "BLURAY": true, "HDTV": true,
	"DVDRIP": true, "BDRIP": true, "DTSHD": true, "EAC3": true,
	"VC1": true, "MPEG2": true, "MPEG4": true,
}
...
// precedingToken 返回紧邻 work[idx] 之前、由 `.`/`-`/`_`/空白分隔的那个片段。
func precedingToken(work string, idx int) string {
	start := idx - 1
	for start >= 0 {
		c := work[start]
		if c == '.' || c == '-' || c == '_' || c == ' ' || c == '\t' {
	...
	return work[start:idx]
}

// isTechnicalToken 判断一个 token 是否是已知技术组件(编码/声道/片源/分辨率/HDR/噪声等)。
func isTechnicalToken(t string) bool {
	if reSSeason.MatchString(t) {
		return true
	}
	for _, re := range []*regexp.Regexp{
		videoRe, audioRe, channelsRe, sourceRe,
		resolutionRe, bitsRe, hdrRe, noiseRe,
	} {
		if re.MatchString(t) {
			return true
		}
	...
	if up == "COMPLETE" || up == "WEB" || up == "REMUX" || t == "全集" {
		return true
	}
	return false
}

// extractGroup 剥离结尾制作组。
//
// 修正历史 bug:旧版把 `.` 也当作制作组分隔符,导致 `AAC2.0.H.264-MWeb` 的
// group 被解析成 `DLAAC20H264MWeb`(吞掉 H264/AAC/2.0)、`DTS-HD.MA.5.1-THOR`
// 被解析成 `HDMA51THOR`(吞掉 MA/5.1)。正确行为:制作组只出现在最后一个 `-` /
// `_` 之后,且该片段既不是已知技术 token,也不属于 `WEB-DL` 这类含连字符技术
// token 的内部。
func extractGroup(work string) (string, string) {
	idx := strings.LastIndexAny(work, "-_")
	if idx < 0 {
		return "", work
	}
	seg := work[idx+1:]
	// 片段必须形如制作组名(字母开头,字母/数字/点/`@`)。
	if !groupTokenShapeRe.MatchString(seg) {
		return "", work
	}
	// 片段本身是技术 token(如 `-AAC`/`-x264`/`-5.1`)→ 不是制作组。
	if isTechnicalToken(seg) {
		return "", work
	}
	// `-`/`_` 属于 `WEB-DL`/`DTS-HD`/`Blu-Ray` 等 token 内部 → 不是制作组。
	if knownHyphenTokens[normTokenKey(precedingToken(work, idx))+normTokenKey(seg)] {
		return "", work
	}
	// 常见非制作组单词(`-V2`/`-Final` 等)。
	if nonGroupSuffixes[strings.ToLower(seg)] {
		return "", work
	}
	return seg, strings.TrimRight(work[:idx], " \t\n\r")
}
